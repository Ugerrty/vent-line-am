...
function initLine(src){
  ...
  var group = new THREE.Group();
  var mesh = new THREE.Mesh(geo, new THREE.MeshMatcapMaterial({
    matcap: matcapTexture(GOLD, [190, 206, 'rgba(18,48,95,0.35)'])
  }));
  group.add(mesh);

  geo.computeBoundingBox();
  var h = geo.boundingBox.max.y - geo.boundingBox.min.y;
  /* тёплое свечение под профилем — свет из щели на тёмном */
  ...
  gg.addColorStop(0, 'rgba(201,169,106,0.55)');
  gg.addColorStop(0.5, 'rgba(201,169,106,0.16)');
  ...
  function pose(p){
    ...
    camera.position.y = 0.85 + bs * 0.75;
    camera.position.z = 8.2 + bs * 3.8;
    camera.lookAt(new THREE.Vector3(cx, -0.05 - bs * 0.15, 0));
